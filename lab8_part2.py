...
class Stepper:
    ...
    def __step(self, dir):
       ...
       #self.s.shiftByte(Stepper.shifter_outputs)
        ...
        #LAB 8 EDIT
        self.step_state += dir    # increment/decrement the step
        self.step_state %= 8      # In range of [0,7]
        Stepper.shifter_outputs &= ~(0b1111 << self.shifter_bit_start)  # reset motor bits
        Stepper.shifter_outputs |= (Stepper.seq[self.step_state] << self.shifter_bit_start)  # set motor bits
        self.s.shiftByte(Stepper.shifter_outputs) # updates shift register
        with self.angle.get_lock():  # synchronize access to shared value
    ...
    def goAngle(self, angle):
         ...
         if delta > 180: 
             ...
             delta += 360
         self.rotate(delta) # Call rotate to move to delta angle
